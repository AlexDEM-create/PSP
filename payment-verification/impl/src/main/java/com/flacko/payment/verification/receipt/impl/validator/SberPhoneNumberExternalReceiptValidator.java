...
@Slf4j
public class SberPhoneNumberExternalReceiptValidator implements ReceiptValidator {
    ...
    private Bank parseBank(String inputBank) {
        Map<String, String> translationMap = new HashMap<>();
        translationMap.put("Тинькофф Банк", "TINKOFF");
        translationMap.put("Альфа Банк", "ALFA");
        translationMap.put("Райффайзен Банк", "RAIFFEISEN");
        StringBuilder translatedBank = new StringBuilder(inputBank);
        for (Map.Entry<String, String> entry : translationMap.entrySet()) {
            String key = entry.getKey();
            String value = entry.getValue();
            translatedBank = new StringBuilder(translatedBank.toString().replace(key, value));
        }
        return Bank.valueOf(translatedBank.toString().toUpperCase());
    }
